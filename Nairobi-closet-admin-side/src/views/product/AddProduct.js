...
const AddProduct = () => {
  ...
  const validateInputs = () => {
    ...
    // err.colors = !colors ? 'Please select one category.' : ''
    err.sizes = !sizes ? 'Please add atleast one size.' : ''
          ...
          <CCard>
            ...
            <CCardBody>
              <CForm className="row g-3 need-validation" onSubmit={handleSubmit}>
                ...
                <CCol md={12}>
                  <CFormLabel htmlFor="sizes">
                    Available Sizes <span style={{ color: 'red' }}>*</span>
                  </CFormLabel>
                  <CFormInput
                    type="text"
                    id="sizes"
                    invalid={errors.sizes.length > 0}
                    placeholder=" Size numbers"
                    value={sizes}
                    onChange={(e) => setSizes(e.target.value)}
                  />
                  {errors && errors.sizes.length > 0 && <InputError>{errors.sizes}</InputError>} 
                </CCol>
                <CCol md={12}>
