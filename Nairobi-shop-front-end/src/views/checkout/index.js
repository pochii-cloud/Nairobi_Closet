...
function Checkout() {
  ...
  const [state, setState] = useState({
    shippingAddress: "",
    paymentMethod: "",
    phone: "", // Define the 'phone' property
    instructions: "" // Define the 'instructions' property
  });

  const handleDeliveryAddress = (value) => {
    setState({ ...state, shippingAddress: value });
  };
  ...
  const handlePlaceOrder = async (e) => {
    e.preventDefault();
    const { shippingAddress, paymentMethod, phone, instructions } = state;
    ...
    let data = {
      ...
      note: "",
      discount: 0,
    };    console.log("This is my big data",data)
    try {
  ...
  const onToken = (token) => {
    ...
    const makePayment = async () => {
      const { shippingAddress, phone, instructions } = state;
      let orderId = Math.floor(
        Math.random() * Math.floor(Math.random() * Date.now())
      );
